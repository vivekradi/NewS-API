...
function DisplayNews() {
  let cartona = `
       <!-- header -->
                    <h2 class="py-3 text-bg-primary rounded-pill"><span class='text-danger'>News source </span>: ${
                      arr.source.name
                    }</h2>
                     <div class='row  my-5 align-items-center  g-4 px-3'>
                     <div class='col-md-6'>
                     <div class='inner in1 rounded-5 py-5'>
                       <h3 class='me-5'><span class='text-danger'>Author</span>:${
                         arr.author
                       }</h3>
                     </div>
                     ...
                     <div class='inner in2 bg-danger rounded-5 py-5'>
                      <h3 class='ms-5'><span class='text-danger'>Title Article</span>: ${
                        arr.title
                      }</h3>
                     </div>
                     </div>
                    ...
                    <!-- Description -->
                    <p id="desc" class="text-bg-secondary p-3 rounded-2 my-5 fs-4 animate__animated animate__shakeY"><span class='text-danger'>Description</span>: ${
                      arr.description
                    }</p>
                    <!-- link and img -->
                    <div class="GoTo d-flex gap-3 justify-content-between align-items-center mt-5">
                        <a target='_blank' href="${
                          arr.url
                        }" id="URL" class="text-bg-warning w-50 h-25 p-3 rounded-2 animate__animated animate__jello">URLTo Read All
                            Article</a>
                        <img  id="photo-news" src="${
                          arr.urlToImage
                        }" class="w-50 ms-5 rounded-3" alt="photo realated by news">
                    </div>
                    <!-- date and next content -->
                    <div class='mt-5'>
                       <p class="pt-5 fs-3">publishedAt : ${Array.from(
                         arr.publishedAt
                       )
                         .join("")
                         .slice(0, 10)}</p>
